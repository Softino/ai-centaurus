...
const RoundTable: React.FC<RoundTableProps> = ({ ownedAgents, allExperts, sentInvitations, onUpdateSession, currentSessions, onCompleteSession }) => {
  ...
  useEffect(() => {
    ...
    const checkAndTriggerTurn = async () => {
      ...
      if (!session || !session.isRunning || isGenerating || session.timeRemaining <= 0 || thinkingAgentId) return;
      ...
      // If text generation is finished, we must also check if AUDIO is finished if in voice mode.
      if (!lastMsg || lastMsg.status === 'approved' || lastMsg.status === 'user') {
          
          // Audio Sync Check: Wait for audio queue to empty before starting next turn.
          // This synchronizes the flow and also naturally limits API rate (Quota Fix).
          if (session.mode === 'voice' && geminiService.isAudioActive()) {
  ...
  const nextAgentTurn = async () => {
    if (!session || isGenerating || !session.isRunning) return;
    
    setHumanInterjectionPending(false);
    setIsGenerating(true);
    setIsWaitingForQuota(false);
    
    const agentIndex = session.currentTurnIndex;
    const agent = session.participants[agentIndex];

    // --- THINKING MODE SIMULATION ---
    // Show thinking state for 1-3 seconds before generating
    setThinkingAgentId(agent.id);
    const thinkingDelay = Math.random() * 2000 + 1000; // 1s to 3s delay
    ...
    try {
      // Determine voice to use (now fully pre-assigned)
      ...
      const fullText = await geminiService.streamAgentResponse(
          ...
          (chunkText) => {
              ...
              onUpdateSession({
                  ...session,
                  messages: [...session.messages, { ...placeholderMessage, text: chunkText }]
              });
          },
          session.mode === 'voice',
          selectedVoice
      ...
      const nextIndex = (session.currentTurnIndex + 1) % session.participants.length;

      // Finalize update with full text (just to be sure)
      onUpdateSession({ 
          ...session, 
          messages: [...session.messages, { ...placeholderMessage, text: fullText }],
          currentTurnIndex: nextIndex,
          currentRound: nextIndex === 0 ? session.currentRound + 1 : session.currentRound
      });
      ...
      const msgs = session.messages.filter(m => m.id !== placeholderId);
      onUpdateSession({
          ...session,
          messages: msgs
      });
            ...
            {session.messages.map(msg => {
                ...
                return (
                  <div key={msg.id} className="flex gap-10 text-right animate-in slide-in-from-bottom-8 duration-700">
                      <div className={`w-16 h-16 rounded-[1.5rem] flex items-center justify-center shrink-0 border-2 transition-all ${
                          isSenderArchitect ? 'bg-indigo-600/20 border-indigo-500 text-indigo-400' : 
                      ...
                      <div className="flex-1 space-y-4">
                          <div className="flex items-center justify-end gap-6">
                              ...
                              <span className={`font-black text-sm uppercase tracking-widest flex items-center gap-2 ${
                                  isSenderArchitect ? 'text-indigo-400' : isSenderExpert ? 'text-emerald-400' : 'text-blue-500'
                              }`}>
                                {msg.agentName}
                          ...
                          <div className={`p-8 rounded-[3rem] border transition-all text-lg leading-relaxed antialiased ${
                              isSenderArchitect ? 'dark:bg-indigo-950/20 bg-indigo-50 dark:border-indigo-800/40 border-indigo-200 dark:text-indigo-100 text-indigo-900 shadow-2xl shadow-indigo-900/10' : 
