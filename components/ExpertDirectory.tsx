          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            {filteredExperts.map(expert => {
              ...
              return (
                <div
                  key={expert.id}
                  className={`group dark:bg-slate-900/40 bg-white border-2 rounded-[3rem] p-10 transition-all duration-500 hover:shadow-2xl flex flex-col relative overflow-hidden ${isAvailable
                ...
                >
                  ...
                  <div className="flex justify-between items-start mb-10">
                    <div className={`w-24 h-24 rounded-[2rem] flex items-center justify-center text-5xl shadow-2xl transition-all duration-500 group-hover:scale-110 ${isAvailable ? 'bg-emerald-600 text-white shadow-emerald-900/30' : 'dark:bg-slate-950 bg-slate-50 dark:text-slate-700 text-slate-300 border dark:border-slate-800 border-slate-200'
                      }`}>
                      <i className={`fas ${expert.icon}`}></i>
                    </div>
                    <div className="text-right flex flex-col items-end">
                      <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-[0.2em] mb-3 border transition-colors ${expert.status === 'available' ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' : 'bg-amber-500/5 text-amber-500/50 border-amber-500/10'
                        }`}>
